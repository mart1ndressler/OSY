...
int s=0;
void s_convert_to_int(char *input) {s = input[1] - '0';}
	...
	for( ;; )
	{
		...
		for ( l_reply_count = 0; pdTRUE; l_reply_count++ )
		{
			...
			if( l_len > 0 )
			{
				...
				if(strncmp(l_rx_buf, "OPEN", 4) == 0)
				{
					open_gate = true;
					s_convert_to_int(l_rx_buf+4);
					xSemaphoreGive(g_sem_led);
				}
				if(strncmp(l_rx_buf, "CLOSE", 5) == 0)
				{
					open_gate = false;
					s_convert_to_int(l_rx_buf+5);
					xSemaphoreGive(g_sem_led);
				}
...
void task_socket_cli( void *tp_arg )
{
	...
	else PRINTF( "Unable to connect server!\r\n" );
...
void task_gate(void* arg)
{
	while(1)
	{
		xSemaphoreTake(g_sem_led, portMAX_DELAY);
		if(open_gate == true)
		{
			close_gate = false;
			for(int i = 0; i < 8; i++)
...
int main(void) {
    ...
    //
	uint8_t ucMAC[ ipMAC_ADDRESS_LENGTH_BYTES ] = { 0x5A, 0xFE, 0xC0, 0xDE, 142, 104 };
	...
	for(int i = 0; i < 8; i++) GPIO_PinWrite(g_led_ptc[ i ].m_led_gpio, g_led_ptc[ i ].m_led_pin, 1);
	if(xTaskCreate(task_gate, "LED_TASK", configMINIMAL_STACK_SIZE + 100, NULL, NORMAL_TASK_PRIORITY, NULL ) != pdPASS ) PRINTF( "Unable to create task '%s'!\r\n", "LED_TASK" );
